...
from yolo_v3.losses import bbox_iou

class BatchGenerator(Sequence):
    def __init__(self, train_inst, base_path, batch_size, phase = 'train', shuffle = True):
    ...
    def resize(self, image, bboxes):
        labels = np.ones((len(bboxes), ))
        aug = Compose([
            Resize(self.input_net_h, self.input_net_w, p=1)
            ], bbox_params={'format': 'pascal_voc', 'label_fields': ['labels']})
        
        aug_result = aug(image=image, bboxes=bboxes, labels = labels)
    ...
    def preprocess_bbox(self, bboxes):
        label = [np.zeros((self.output_net_h[i], self.output_net_w[i], self.anchor_per_scale,
    ...
    def __getitem__(self, idx):     
        ...
        for ind, i in enumerate(range(l_bound, r_bound)):   
            ...
            img = self.load_img(f"{self.base_path}/{self.train_inst[i]['file_name']}")
            bbox = [obj['bbox'] for obj in self.train_inst[i]['bboxes']]
            if self.phase == 'train':
                img, bbox = self.apply_aug(img, bbox)
            else:
                img, bbox = self.resize(img, bbox)
            bbox = [list(box) + [obj['category_id']-1] for box, obj in zip(bbox, self.train_inst[i]['bboxes'])]
        ...
        return [batch_image/255.0, batch_label_sbbox, batch_label_mbbox, batch_label_lbbox, batch_sbboxes, batch_mbboxes, batch_lbboxes], np.zeros(self.batch_size)